    ...
    createServer(port) {
        ...
        const tcpServer = net.createServer((socket) => {
            ...
            try {
                ...
                console.error('Error initializing terminal classes:', error);
                socket.destroy(); // Закрываем сокет, если произошла ошибка
            }

            // Обработка ошибок на уровне сокета
            socket.on('error', (err) => {
                console.error(`Socket error from ${socket.remoteAddress}:`, err);
                socket.destroy(); // Закрываем сокет при ошибке
            });
        });
