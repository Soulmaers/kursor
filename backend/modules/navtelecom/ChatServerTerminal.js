        ...
        for (let i = 0; i < count; i++) {
            ...
            for (let j = 0; j < this.bits.length; j++) {
                ...
                switch (this.bits[j]) {
                    ...
                    case 29:
                        val = buf.readUInt8()
                        global.in1 = BitsCount.check(val, 0) ? 'Вход IN1(датчик сработал)' : 'Вход IN1(датчик в нормальном состоянии)'
                        global.in2 = BitsCount.check(val, 1) ? 'Вход IN2(датчик сработал)' : 'Вход IN2(датчик в нормальном состоянии)'
                        global.in3 = BitsCount.check(val, 2) ? 'Вход IN3(датчик сработал)' : 'Вход IN3(датчик в нормальном состоянии)'
                        global.in4 = BitsCount.check(val, 3) ? 'Вход IN4/AIN1/AIN2(датчик сработал)' : 'Вход IN4/AIN1/AIN2(датчик в нормальном состоянии)'
                        global.in5 = BitsCount.check(val, 4) ? 'Вход IN5/AIN3(датчик сработал)' : 'Вход IN5/AIN3(датчик в нормальном состоянии)'
                        global.in6 = BitsCount.check(val, 5) ? 'Вход IN6/SH1(датчик сработал)' : 'Вход IN6/SH1(датчик в нормальном состоянии)'
                        global.in7 = BitsCount.check(val, 6) ? 'Вход IN7/SH2(датчик сработал)' : 'Вход IN7/SH2(датчик в нормальном состоянии)'
                        global.in8 = BitsCount.check(val, 7) ? 'Вход IN8/SH3-SH4(датчик сработал)' : 'Вход IN8/SH3-SH4(датчик в нормальном состоянии)'
                        buf = buf.slice(1)
                        break;
                    case 30:
                        val = buf.readUInt8()
            ...
            function funct(buf, index) {
                ...
                buf = buf.slice(1)
                val = buf.readUInt8()
                global[`tpms_pressure_${index}`] = val !== 0 ? val * 0.1 : 0
                buf = buf.slice(1)
                val = buf.readInt8()
