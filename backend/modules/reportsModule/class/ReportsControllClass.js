...
class ReportsControllClass {
    ...
    async startCalculate() {
        ...
        const instanceRefill = new OilCalculator(this.data, this.setAttributes, this.idObject)
        const refill = await instanceRefill.init()
        const instanceDrain = new DrainCalculate(this.data, this.setAttributes, this.idObject)
        const drain = await instanceDrain.init()
        const startOil = this.data[0].dut ? (CalculateReports.startAndFinishOil(this.data))[0] : 'Н/Д'
        const zapravleno = this.data[0].dut ? refill[0] : 'Н/Д'
        const countZapravka = this.data[0].dut ? refill[1] : []
        const slito = this.data[0].dut ? drain[0] : 'Н/Д'
        const countSliv = this.data[0].dut ? drain[1] : []
        const finishOil = this.data[0].dut ? (CalculateReports.startAndFinishOil(this.data))[1] : 'Н/Д'
        const mileage = this.data[0].mileage ? parseFloat(CalculateReports.calculationMileage(this.data).toFixed(2)) : 'Н/Д'
        const traveling = CalculateReports.traveling(this.data, this.setAttributes)
        const parkings = CalculateReports.parkings(this.data, this.setAttributes)
        const stops = CalculateReports.stops(this.data, this.setAttributes)
        const moto = await CalculateReports.moto(this.data, this.setAttributes, this.idObject)
        this.instance = new ProstoyControll(this.data, this.typeIndex, this.setAttributes, this.idObject)
        const prostoy = this.instance.init()
        //  console.log(moto.moto[0])
        this.instanceSKDSH = new SCDSHClass(this.object)
        const skdsh = await this.instanceSKDSH.init()

        const rashodDUT = this.data[0].dut ? parseFloat((startOil + zapravleno - finishOil).toFixed(2)) : 'Н/Д'
        const rashodDUTKM = this.data[0].dut && this.data[0].mileage ? parseFloat(((rashodDUT / mileage) * 100).toFixed(2)) : 'Н/Д'
        const rashodDUTMCH = this.data[0].dut && moto.motoAll !== 0 ? parseFloat(((rashodDUT / moto.motoAll) * 3600).toFixed(2)) : 'Н/Д'
        ...
        this.attributes.component['Поездки'].forEach(e => { if (e.name === 'Длительность') { e.result = traveling.map(e => CalculateReports.formatTime(e[2].time)), e.local = 'чч:мм:сс' } })
        ...
        this.attributes.component['Стоянки'].forEach(e => { if (e.name === 'Длительность') { e.result = parkings.map(e => CalculateReports.formatTime(e.diff)), e.local = 'чч:мм:сс' } })
        ...
        this.attributes.component['Остановки'].forEach(e => { if (e.name === 'Длительность') { e.result = stops.map(e => CalculateReports.formatTime(e.diff)), e.local = 'чч:мм:сс' } })
    ...
    async getSetAttributesTemplates() {
        const result = await JobToBase.getSettingsToBase(this.idObject)
