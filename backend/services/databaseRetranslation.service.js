...
exports.updateFlagForExistingObjects = async (uniqRetraID, table) => {
    const pool = await connection;
    const updateFlagQuery = `
        UPDATE ${table} SET flag = 'false' WHERE incriment_retra = @uniqRetraID`;
...
exports.addGroups = async (object) => {
    ...
    try {
        ...
        const pool = await connection;
        const query = `SELECT idx FROM groups WHERE idx=@idx AND incriment_retra=@incriment_retra`;
        const result = await pool.request()
            .input('idx', String(object.idx))
            .input('incriment_retra', String(uniqRetraID))
            .query(query)
        ...
        if (result.recordset.length > 0) {
            const updateFlagQuery = `
        UPDATE groups SET flag = 'true' WHERE idx=@idx AND incriment_retra = @uniqRetraID`;
            await pool.request()
                .input('uniqRetraID', uniqRetraID)
                .input('idx', String(object.idx))
                .query(updateFlagQuery);
