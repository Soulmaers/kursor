...
export function createMelagiTS(data, num, container) {
    const chartJobTS = container.querySelector('.chartJobTS')
    chartJobTS.style.width = '700px'
    chartJobTS.style.alignItems = 'center'
    const intervalTitle = container.querySelector('.intervalTitle')
    intervalTitle.style.fontSize = '16px'
    const chartStatic = container.querySelector(`.chartStatic${num}`)
    if (chartStatic) {
        chartStatic.remove();
    }
    const jobTSDetalisationLine = container.querySelector('.jobTSDetalisationLine')
...
export function createOilTS(data, num, container) {
    const jobTSDetalisationChartsLegenda = container.querySelector('.jobTSDetalisationCharts_legenda')
    const legendaButton = container.querySelectorAll('.legendaButton')
    ...
    const dat = Object.values(data)[0]
    const dataArray = Object.entries(dat).map(([category, value]) => ({ category, value }));
    // Задание размеров графика
    const chartJobTS = container.querySelector('.chartJobTS')
    var widthWind = document.querySelector('body').offsetWidth;
    const jobTSDetalisationLines = container.querySelector('.jobTSDetalisationLine')

    jobTSDetalisationLines.style.width = ''
    jobTSDetalisationLines.style.width = widthWind / 3 //widthWind / 3 - 15
    chartJobTS.style.alignItems = 'center';
    const width = widthWind < 860 ? widthWind / 3 - 15 : container.clientWidth / 3 - 40//widthInPx; // Ширина графика
    // var width = 220;
    var height = 200;
    ...
    var maxValue = d3.max(Object.entries(dat).map(item => item[1]));
...
export function createJobTS(data, num, container) {
    ...
    const chartStatic = container.querySelector(`.chartStatic${num}`)
    console.log(chartStatic)
    if (chartStatic) chartStatic.remove();
    ...
    const chartJobTS = container.querySelector('.jobTSDetalisationGraf')
    var widthWind = document.querySelector('body').offsetWidth;
    const jobTSDetalisationLines = container.querySelector('.jobTSDetalisationLine')
    jobTSDetalisationLines.style.width = ''
    jobTSDetalisationLines.style.width = widthWind / 3 //widthWind / 3 - 15
...
export function createChart(data, num, container) {
    ...
    const combinedData = combineIntervals(data[0]);
    const win = container.clientWidth == 0 ? 707 : container.clientWidth
    ...
    const xScale = d3.scaleTime()
        .domain(d3.extent(data[0], d => new Date(d.time)))
        .range([0, width]);
